...
import usePageViews from "./hooks/Redirection";
...
const authSendDataRequest = () => {

  const expectedDataServer = {
  ...
  return sendAxiosRequest({
    method: "post",
    url: "https://test.bridesfactory.gorakoda.com/api/v0/dashboards/auth/login",
    responseType: 'json',
    headers: {"Content-Type": "application/json; charset=UTF-8"},
    data: JSON.stringify(expectedDataServer),
  }).then(response => {
  ...
  <BrowserRouter>
    <React.Suspense fallback={<Loader/>}>
      <Routes>
        ...
        {/* //http://localhost:3000/pages/emptypage/ */}
        {/* {check ?   <Route
